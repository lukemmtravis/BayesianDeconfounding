...
%\subparspace
%{\bf High-dimensional sparse linear regression.} 
...
\begin{definition}[Deconfounded model selection prior]\label{def:decon_model_selection_prior}
...
\begin{definition}[Deconfounded Gaussian process prior]\label{def:decon_GP}
...
%L^{-1}(L^{-1})^T = (L^T L)^{-1} = (I - X\Sigma_*X^T)^{-1} = 
...
%\subsubsection*{Full Posterior Example}
\subparspace
...
%%% Removed - evidence of correlation removal %%%
...
%\subsubsection*{Doubly debiased uncertainty quantification}
...
%\subsubsection*{Riboflavin dataset}
...
\begin{tabular}{l|r|c|ccc}
\toprule
Scenario $(n, p, s_0, q, \beta^0_*, \beta_i^0, \delta_i^0)$               & Method & Cov.  & MAE                          & Length              & Time\\
\hline
